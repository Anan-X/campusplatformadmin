  ...
  >
    <div class="demo-drawer__content">
      <el-form :model="form">
        <el-form-item label="课节" :label-width="formLabelWidth">
          <el-select v-model="form.time_id" placeholder="请选择课节">
            <div v-if="freetime_id.length == 0 ? true : false">
              <el-option
                :label="subjectTime(freetime_id)"
                :value="freetime_id"
              ></el-option>
            </div>
            <div v-else>
              <el-option
              v-for="item in freetime_id"
              :key="item"
              :label="subjectTime(item)"
              :value="item"
            ></el-option>
            </div>
          </el-select>
      ...
      <div class="demo-drawer__footer">
        ...
        <el-button type="primary" @click="onSubmitEdit" :loading="loading">{{
          loading ? "提交中 ..." : "确 定"
        }}</el-button>
      </div>
    </div>
...
<script>
import { apiGetsta, apiEditCourse } from "network/user";
export default {
  props: {
    dialog: Boolean,
    formEdit: Object,
  ...
  methods: {
    ...
    onSubmitEdit() {
      ...
      this.loading = true;
      if (
        Object.values(this.form).find((i) => {
          return i == "" || i == null;
        }) != undefined
      ) {
        this.$message("信息请填全");
      } else {
        apiEditCourse(this.form).then(
          (res) => {
            console.log(res)
